...
6. [Sample vs Population](#sample-vs-population)
    1. [Sample Mean](#sm)
    2. [Sample Variance](#sv)
        1. [Bessel's Correction](#besscorr)
        2. [Bessel Correction Intuitive Explanation](#bessel_correction_intuitive_explanation)
        3. [Other scenarios w.r.t. DOF](#dof_other_scenarios)
...
8. [Tricks used as part of this README](#tricks)
...
- using Maclaurin series (Taylor series for x=0, here t=0):
    ...
    - interchange the summation and integral using [Fubini's Theorem](#fubini)
        ...
        - this interchange is guaranteed only when the summation is guaranteed to have an **absolute convergence**, i.e. $\int\limits_{-\infty}^{\infty}\left(\sum\limits_{n=1}^{\infty} \left|\dfrac{(Xt)^n}{n!}\right| \right) f_X(x) < \infty$.
...
- as per **CLT** (which makes $\bar{X}$ normally distributed), $\large \bar{X} \sim \mathcal{N}\left(\mu, \dfrac{\sigma^2}{n}\right)$
...
### Intuitive explanation of this correction term<a name="bessel_correction_intuitive_explanation"></a>
...
### Other scenarios w.r.t. DOF<a name="dof_other_scenarios"></a>
...
# Tricks used as part of this README<a name="tricks"></a>

## Integration
1. for $x^2\,, \, y^2 \rightarrow x = rcos\theta\,,\, y = rsin\theta$
2. Convert equations of type $ax^2 + bx$ to whole squares: $ax^2 + bx = \left((\sqrt{a}x)^2 + 2.(\sqrt{a}x).(\dfrac{b}{2\sqrt{a}}) + \dfrac{b^2}{4a}\right) - \dfrac{b^2}{4a} = \left(x\sqrt{a} + \dfrac{b}{2\sqrt{a}}\right)^2 - \dfrac{b^2}{4a}$
3. Integration by parts
4. $tan(\theta)$ based substitution: $\int\dfrac{1}{1+u^2}du \rightarrow u = tan(\theta) \,,\, du = sec^2(\theta)d\theta \Rightarrow \int\dfrac{1}{sec^2(\theta)}sec^2(\theta)d\theta = \theta = tan^{-1}(u)$